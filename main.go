...
func formatMessage(payload WebhookPayload) string {
    ...
    parsedURL, err := url.Parse(resource.ResourceURL)
    if err != nil {
        log.Printf("Error parsing resource URL: %s", err)
        return "Error formatting message: Invalid resource URL"
    }
    harborHost := parsedURL.Hostname()
